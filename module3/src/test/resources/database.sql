...
CREATE TABLE IF NOT EXISTS `gift_certificates`.`order`
(
    `id`                   BIGINT         NOT NULL AUTO_INCREMENT,
    `timestamp`            DATETIME       NOT NULL,
    `cost`                 DECIMAL(10, 0) NOT NULL,
    `user_id`              BIGINT         NOT NULL,
    `order_certificate_id` BIGINT         NOT NULL,
    PRIMARY KEY (`id`),
    foreign key (`user_id`) references `gift_certificates`.`user` (`id`) on delete cascade,
    foreign key (`order_certificate_id`) references `gift_certificates`.`gift_certificate` (`id`) on delete cascade
